...
setup() {
  rm -rf "$root/remote" "$root/ws" "$root/publish" "$root/other" "$root/hook-ran"
  git init -q --bare -b main "$root/remote"
  git clone -q "$root/remote" "$root/ws" 2>/dev/null
  cd "$root/ws"
...
setup; echo evil > evil.txt; git add evil.txt
publish; expect_published "ignores unrelated staged file" $?

setup; echo tampered > README.md
publish; expect_published "ignores unrelated unstaged edit" $?

setup; echo evil > evil.txt; git add evil.txt; git "${G[@]}" commit -q -m sneaky
publish; expect_published "ignores commit made in the workspace" $?

setup
git clone -q "$root/remote" "$root/other" 2>/dev/null
(cd "$root/other" && echo other > other.txt && git add other.txt &&
  git "${G[@]}" commit -q -m "someone else" && git push -q origin main 2>/dev/null)
...
setup; before=$(git --git-dir="$root/remote" rev-parse main)
rm docs/architecture.md; ln -s /etc/hostname docs/architecture.md
publish; expect_refused "refuses symlinked doc" $? "$before"

setup; before=$(git --git-dir="$root/remote" rev-parse main)
mv docs "$root/real-docs"; ln -s "$root/real-docs" docs
publish; expect_refused "refuses symlinked docs directory" $? "$before"
rm -rf "$root/real-docs"

setup; git add docs; git "${G[@]}" commit -q -m "docs already there"; git push -q origin main 2>/dev/null
...
chmod +x "$root/remote/hooks/pre-receive"
